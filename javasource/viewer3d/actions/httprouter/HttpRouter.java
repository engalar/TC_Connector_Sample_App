...
public class HttpRouter {
	...
	public HttpRouter() {
		this.logger = Core.getLogger(HttpRouter.class.getSimpleName());
	...
	public void processRequest(IContext ctx, IMxRuntimeRequest request, IMxRuntimeResponse response, String path) {
		...
		try {
			...
			response.setStatus(IMxRuntimeResponse.INTERNAL_SERVER_ERROR, ex.getCause().getMessage());
		} catch (Exception ex) {
			this.logger.error(ex);
			response.setStatus(IMxRuntimeResponse.INTERNAL_SERVER_ERROR, ex.getMessage());
		}
	}
